...
public class BookRoomCommandHandler : ICommandHandler<BookRoomCommand, BookRoomCommandResponse>
{
    ...
    public async Task<Result<BookRoomCommandResponse>> Handle(BookRoomCommand request, CancellationToken cancellationToken)
    {
        ...
        RoomDto? room = await _roomRepository.GetById(request.RoomId, cancellationToken);
        ...
        BookingTimeDiscountDto? discountDto = await _bookingTimeDiscountRepository.GetDiscount(bookTime, cancellationToken);
        ...
        var booking = BookingDto.Create(request.RoomId, request.Start, end, price, request.Services);
